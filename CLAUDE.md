...
- Phase 8 Service+Grades+Me complete. 267 unit tests. `lib/liffOpen.ts` (LIFF isInClient / window.open fallback) · `fixtures/service.ts` (6 ServiceItem with external URLs) · `ServiceGrid` (2-col grid, onPress(url)) · `fixtures/grades.ts` (SemesterGrade, CourseGrade, GRADE_COLOR, isLocked gate) · `SemesterSelector` + `CourseGradeRow` + `LockedNotice` · `ProfileCard` + `StatsRow` + `SettingsSection` · ServiceView / GradesView (useState selectedId) / MeView all wired.
- Phase 9 LINE push + Vercel Cron complete. 293 unit tests. `lib/line-messaging.ts` (createLineMessaging(deps) factory · push/multicast/broadcast · LINE_CHANNEL_ACCESS_TOKEN dev fallback · injected fetcher for tests) · `lib/line-webhook.ts` (HMAC-SHA-256 base64 verify · timingSafeEqual constant-time compare) · `lib/duty-roster.ts` (fixture-backed; Phase 10 swaps to Prisma DutyShift) · `app/api/line/webhook` (factory+handler split mirroring auth/line/callback · 503 env / 401 bad-sig / 400 bad-body / 200 + AuditLog per event) · `app/api/cron/duty-reminder` (GET+POST · Bearer ${CRON_SECRET} gate · multicast today's cadets · single AuditLog summary · injected now()) · `vercel.json` crons `0 22 * * *` · middleware PUBLIC_PREFIXES adds `/api/line` + `/api/cron` (each handler authorises itself, so cookie middleware bypasses).
- Phase 10 Hardening complete. 316 unit tests. `lib/security-headers.ts` (SECURITY_HEADERS list: HSTS preload · X-Frame DENY · X-Content-Type nosniff · Referrer-Policy strict-origin-when-cross-origin · Permissions-Policy locks camera/mic/geolocation/payment/usb · COOP same-origin · CORP same-origin · CSP `frame-ancestors 'none'` + LIFF CDN allow) wired via `next.config.ts` headers() · `lib/rbac.canExportAudit` (ADMIN only) · `GET /api/me/export` (PDPA self-service: User row with decrypted email + every AuditLog row, AES-GCM decrypt errors surface as `emailDecryptError` in bundle) · `POST /api/me/delete` (revoke all live RefreshToken rows + clearAuthCookies()) · `GET /api/admin/audit/export` (CSV stream · `?from=YYYY-MM-DD&to=YYYY-MM-DD&limit=N` · strict date parsing · CSV escape doubles `"` and wraps cells with `,"\n\r`) · `lighthouserc.json` (mobile preset · perf ≥ 0.85 · LCP ≤ 2.5s · TBT ≤ 200ms · JS ≤ 250KB · CSS ≤ 50KB) · `THREAT_MODEL.md` documents 10 threats (T1–T10) with controls + residual risk.
- Milestone v1 web/LIFF rebuild complete: 10 phases · 316 unit tests + 8 Playwright chassis specs · all bilingual READMEs mirrored · `tsc --noEmit` clean · build green.
