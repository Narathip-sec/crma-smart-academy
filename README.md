...
| 2026-05-20 | Phase 5 — ClassScheduleView                      | `lib/courseTypeColor.ts` + `lib/formatMilTime.ts` (ports 1:1) · `fixtures/schedule.ts` (7 classes · WED empty · todayKey) · `useScheduleStore` (Zustand) · `DaySwitcher` (slate-800 pill · amber-400 active) · `SemesterPill` (amber pill + transparent `<select>`) · `ClassCard` (colour bar · time pill · type badge) · ClassScheduleView wired. `DESIGN_BRIEF.txt` exported at repo root. 207 unit tests green. |
| 2026-05-20 | Phase 6 — HealthView                             | `fixtures/health.ts` (7-day · ActivityType) · `useHealthStore` (selectedDate · waterCups · add/remove) · `WeekCalendarStrip` (7-circle SVG rings · step progress) · `StepsCard` (donut SVG) · `WaterLogger` (cup icons · +/−) · `CaloriesCard` (consumed / burned / goal / net) · `ActivityLog` (type icons · empty state) · HealthView wired. 230 unit tests green. |
| 2026-05-20 | Phase 7 — ActivityView                           | `fixtures/activity.ts` (ActivityEvent · Announcement · EventCategory · AnnouncementLevel) · `useActivityStore` (activeTab · rsvpSet · toggleRsvp) · `TopTabs` · `AttendeesStack` (overlapping initials · overflow count) · `EventCard` (category badge · RSVP toggle · aria-pressed) · `AnnouncementCard` (level badge) · ActivityView wired. 249 unit tests green. |
| 2026-05-20 | Phase 8 — Service + Grades + Me                  | `lib/liffOpen.ts` (LIFF / window.open fallback) · `ServiceGrid` (6-tile grid · external URLs) · `SemesterSelector` + `CourseGradeRow` + `LockedNotice` (isLocked gate) · `ProfileCard` (initials avatar) · `StatsRow` (GPA/PFT/credits) · `SettingsSection` (4 items · onPress) · all 3 views wired. 267 unit tests green. |
| 2026-05-20 | Phase 9 — Push (LINE Messaging) + Vercel Cron    | `lib/line-messaging.ts` (push · multicast · broadcast · DI fetcher · dev fallback when `LINE_CHANNEL_ACCESS_TOKEN` unset) · `lib/line-webhook.ts` (HMAC-SHA-256 `x-line-signature` verify · constant-time compare) · `lib/duty-roster.ts` (4-cadet fixture) · `POST /api/line/webhook` (503 env / 401 bad-sig / 400 bad-body / 200 + AuditLog per event) · `GET POST /api/cron/duty-reminder` (Bearer `${CRON_SECRET}` gate · multicast to today's cadets · single AuditLog summary) · `vercel.json` crons `0 22 * * *` · middleware `/api/line` + `/api/cron` public prefixes. 293 unit tests green. |
| 2026-05-20 | Phase 10 — Hardening                             | `lib/security-headers.ts` (CSP / HSTS-preload / X-Frame DENY / Permissions-Policy / COOP / CORP) wired via `next.config.ts` headers() · `lib/rbac.canExportAudit` (ADMIN-only) · `GET /api/me/export` (PDPA self-export: User row + decrypted email + every AuditLog row keyed to caller · `Content-Disposition: attachment`) · `POST /api/me/delete` (revokes all live `RefreshToken` rows, clears auth cookies, `AuditLog` row) · `GET /api/admin/audit/export` (CSV stream · RBAC-gated · `?from`/`?to`/`?limit` filters · CSV-escape pass) · `lighthouserc.json` (perf ≥ 0.85, LCP ≤ 2.5s, JS bundle ≤ 250KB, CLS ≤ 0.1) · `THREAT_MODEL.md` documents 10 threats with controls. 316 unit tests green. |
