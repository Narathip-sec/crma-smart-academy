...
import { createTotpReverifyHandler } from '@/app/api/auth/totp/reverify/handler'
import {
  ACCESS_COOKIE,
  ENROL_COOKIE,
  REFRESH_COOKIE,
...
function setup(opts?: {
  ...
  const consumeCode = vi.fn((): ConsumeResult => opts?.consumeResult ?? { ok: true, step: 200n })

  const handler = createTotpReverifyHandler({
...
async function postWithCookie(body: unknown, cookie?: string) {
  const req = new NextRequest(new URL('http://localhost/api/auth/totp/reverify'), {
    method: 'POST',
...
describe('POST /api/auth/totp/reverify — enrol cookie gate', () => {
...
describe('POST /api/auth/totp/reverify — body validation', () => {
...
describe('POST /api/auth/totp/reverify — enrol state', () => {
  test('user missing → 409 not_enrolled', async () => {
    const token = await signEnrolToken(ENROL)
  ...
  test('user.totpVerified null → 409 not_enrolled (must complete Phase 2d first)', async () => {
    const token = await signEnrolToken(ENROL)
    const { handler } = setup({ user: { totpVerified: null } })
    const res = await handler(await postWithCookie({ code: '123456' }, token))
    expect(res.status).toBe(409)
    expect((await res.json()).error).toBe('not_enrolled')
  })
...
describe('POST /api/auth/totp/reverify — code outcomes', () => {
  test('invalid_code → 401 (no state change)', async () => {
    const token = await signEnrolToken(ENROL)
    ...
    expect(baseUser.lastTotpStep).toBe(100n)
    expect(res.headers.get('set-cookie')).toBeFalsy()
  })

  test('replay → 409 (no state change)', async () => {
    const token = await signEnrolToken(ENROL)
    const { handler, prisma, baseUser } = setup({
...
describe('POST /api/auth/totp/reverify — happy path', () => {
  test('valid code → 200 ok + lastTotpStep advanced + RefreshToken row + 3 Set-Cookies', async () => {
    const token = await signEnrolToken(ENROL)
    const { handler, prisma, audit, baseUser } = setup({
      consumeResult: { ok: true, step: 250n },
    })
    const res = await handler(await postWithCookie({ code: '123456' }, token))
    ...
    const upd = prisma.user.update.mock.calls[0]?.[0] as {
      data: { lastTotpStep: bigint; totpVerified?: Date }
    }
    expect(upd.data.lastTotpStep).toBe(250n)
    // Reverify must NOT re-stamp totpVerified.
    expect(upd.data.totpVerified).toBeUndefined()
    expect(baseUser.lastTotpStep).toBe(250n)

    expect(prisma.refreshToken.create).toHaveBeenCalledTimes(1)
    const rt = prisma.refreshToken.create.mock.calls[0]?.[0] as {
      data: { userId: string; deviceFp: string }
    }
    expect(rt.data.userId).toBe('user_1')
    expect(rt.data.deviceFp).toBe('devfp_new')
  ...
  test('reverify accepts lastTotpStep=null (cadet who skipped Phase 2e migration)', async () => {
    const token = await signEnrolToken(ENROL)
    const { handler } = setup({ user: { lastTotpStep: null } })
    const res = await handler(await postWithCookie({ code: '123456' }, token))
    expect(res.status).toBe(200)
  })
})
