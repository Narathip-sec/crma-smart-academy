...
### D2 — Final verification sweep ☑ (done — Sonnet 5, 2026-07-21: (1) lint+tsc+build all green. (2) All ~20 routes 200 OK via fetch smoke test (static + dynamic /activity/[id], /announcements/[id]); dark theme verified via computed CSS tokens (data-theme=dark → --bg/--ink/--surface all swap correctly), spot-checked on home + settings/pdpa. (3) Security re-check: ran `next build && next start` under real NODE_ENV=production with NEXT_PUBLIC_LIFF_ID unset — GET /api/me with no cookie returned 404 instead of 401, a pre-existing bug (route checked `!identity` but returned "user not found"/404 instead of 401 unauthenticated) — **fixed** (src/app/api/me/route.ts now returns 401). Re-verified: all 14 protected routes now 401 with no cookie in true production mode. Switched to dev server for authenticated checks: evil photoUrl → 400, bogus todo id complete → 404, activity detail has attendeeCount+myRsvp only (no raw attendees array). (4) Guardrails: lib/data/ has only csv-schema.ts; grepped full session diff for new hex (only pre-existing #fff pattern) and font sizes outside {11,13,15,20} — found and **fixed** one genuine violation (settings/pdpa/page.tsx used 12px, changed to 13px); the two other 12px hits were pre-existing code relocated by U3, not new. (5) Updated CLAUDE.md route list (was missing /settings/pdpa from U4), PRESENTATION-PREP-PLAN.md (Playbook 2 completion summary), and added a Known Limitations section to NEXT-STEPS-SUMMARY-2026-07-16-TH.md (rate limiting, session revocation, activity auto-approve, upload-to-record binding, partial zod-style validation). (6) Pushed to origin/main, verified production.)
