...
### T11 — Touch feedback + tap targets ☑ (done by Sonnet 5 2026-07-09 — kit primitives (Button, Chip, Seg, states.tsx retry, ListItem already had it) + shell (AppBar back, BottomNav, TopBar) get active:opacity-70 once, cascading to every swept page; grid/promo tiles (quick-services, hero-carousel card, service Recent/Popular) get active:scale-[0.98]; remaining hand-rolled back buttons/FABs/nav-arrows/day-tabs across ~20 page files get active:opacity-70; BottomNav Links widened to fill full h-16 (was content-height only, under 44px) via items-stretch; todo done-toggle and sheet close (✕) button hit areas enlarged to 44×44 via negative-margin wrapper around the small visual glyph; disabled-during-submit already verified present on report/activity-new/lost-found-new/todo-sheet from earlier tasks)

### T12 — Loading/empty/error consistency ☑ (done by Sonnet 5 2026-07-09 — gated-fetch pages wired to LoadingState/EmptyState/ErrorState with retry: /activity, /activity/[id], /calendar, /lost-found, /lost-found/[id], /meals (kept its documented cancelled+keyed pattern, added error branch), /report/tickets, /todo (previously had zero loading/error handling — items:null initial state added). Form pages with fallback-array enrichment fetches (activity/new, lost-found/new, report, profile, home/MyDay) don't gate render on the fetch so were left as graceful-degrade only; /profile got a stray-promise `.catch` no-op. All new `load()` callbacks keep setState only inside `.then`/`.catch` (never synchronously in the effect body) per the project's set-state-in-effect lint rule — discovered mid-task when the sync-reset pattern tripped `react-hooks/set-state-in-effect` on 6 files.)

### T13 — Dead code cleanup ☑ (done by Sonnet 5 2026-07-09 — verified zero imports via grep, deleted `src/lib/data/activity.ts`; active-routes list in `CLAUDE.md` was also missing `/lost-found/new` alongside `/announcements`/`/announcements/[id]` — added all three since the list's purpose is accuracy; `mock-data.ts` untouched)

### T14 — Upload endpoint auth (security, audit C2) ☑ (done by Sonnet 5 2026-07-09 — `src/app/api/upload/route.ts` POST now resolves the dev user via the identical `DEV_EMAIL` + `prisma.user.findUnique` pattern used by 8 other routes, returns 401 when absent, before ever parsing the request body; `allowedContentTypes` restricted to jpeg/png/webp (dropped heic); `maximumSizeInBytes` set to 5MB; `tokenPayload` now carries `userId`. Verified live via curl against `/api/upload`'s `blob.generate-client-token` path: auth gate passes through to `handleUpload` correctly (non-401), which then fails only on a separate pre-existing local gap — no `BLOB_READ_WRITE_TOKEN` configured in this dev `.env` at all, unrelated to this change and not something I have the credential to fix. Also surfaced along the way: local Prisma client is generated `--no-engine` (Accelerate-only) per `package.json`, so the committed `.env`'s plain `postgres://` `DATABASE_URL` can never connect locally as-is — regenerated the client without `--no-engine` for one session to verify, then restored `--no-engine` before finishing so the tree matches the committed build config. Did not touch `.env`, `package.json`, or Blob credentials — flagged for the user, not fixed.)
