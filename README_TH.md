...
| 2026-05-20 | Phase 8 — Service + Grades + Me                  | `lib/liffOpen.ts` (LIFF / window.open fallback) · `ServiceGrid` (6 กระเบื้อง · URL ภายนอก) · `SemesterSelector` + `CourseGradeRow` + `LockedNotice` (gate isLocked) · `ProfileCard` (avatar initials) · `StatsRow` (GPA/PFT/credits) · `SettingsSection` (4 รายการ) · 3 view ครบ · unit 267 ผ่าน |
| 2026-05-20 | Phase 9 — Push (LINE Messaging) + Vercel Cron    | `lib/line-messaging.ts` (push · multicast · broadcast · DI fetcher · dev fallback) · `lib/line-webhook.ts` (ตรวจ HMAC-SHA-256 ของ `x-line-signature` แบบ constant-time) · `lib/duty-roster.ts` (fixture 4 นนร.) · `POST /api/line/webhook` (503/401/400/200 + AuditLog ต่อ event) · `GET POST /api/cron/duty-reminder` (gate `Bearer ${CRON_SECRET}` · multicast ถึงเวรประจำวัน · AuditLog สรุปครั้งเดียว) · `vercel.json` crons `0 22 * * *` · middleware ปลดล็อก `/api/line` + `/api/cron` · unit 293 ผ่าน |
| 2026-05-20 | Phase 10 — Hardening                             | `lib/security-headers.ts` (CSP / HSTS-preload / X-Frame DENY / Permissions-Policy / COOP / CORP) ผูกผ่าน `next.config.ts` headers() · `lib/rbac.canExportAudit` (เฉพาะ ADMIN) · `GET /api/me/export` (PDPA self-export: User + email ที่ถอดรหัส + AuditLog ของผู้ใช้ · `Content-Disposition: attachment`) · `POST /api/me/delete` (revoke `RefreshToken` ที่ยังใช้งานทุกแถว · ล้างคุกกี้ auth · เขียน AuditLog) · `GET /api/admin/audit/export` (สตรีม CSV · gate RBAC · filter `?from`/`?to`/`?limit` · escape CSV) · `lighthouserc.json` (perf ≥ 0.85 · LCP ≤ 2.5s · JS bundle ≤ 250KB · CLS ≤ 0.1) · `THREAT_MODEL.md` (10 ภัยคุกคามพร้อม mitigations) · unit 316 ผ่าน |
